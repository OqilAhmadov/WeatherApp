import { WeatherState, WeatherAction, GET_WEATHER_REQUEST, GET_WEATHER, SET_LOADING, SET_ERROR } from '../types';        // import the types
...
export const WeatherReducer = (state = initialState, action: WeatherAction): WeatherState => {  // create the weather reducer
    switch (action.type) {  
        case GET_WEATHER_REQUEST:                                                              // if the action is get weather
        return {
            data: null,
            loading: true,
            error: ""
